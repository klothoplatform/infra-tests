...
def test_write_read_kv_entry():
    ...
    response = requests.post(primary_gw_url("test/persist-orm/typeorm-write-kv-entry"),
                             json=entry)
    assert response.status_code == 200

    response = requests.get(primary_gw_url("test/persist-orm/typeorm-read-kv-entry"),
