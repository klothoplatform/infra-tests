...
def test_write_read_kv_entry():
    ...
    response = requests.post(urljoin(primary_gw_url, os.path.join(primary_gw_stage,"test/persist-orm/sequelize-write-kv-entry")),
                             json=entry)
    assert response.status_code == 200

    response = requests.get(urljoin(primary_gw_url, os.path.join(primary_gw_stage,"test/persist-orm/sequelize-read-kv-entry")),
