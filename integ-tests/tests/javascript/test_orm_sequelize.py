...
def test_write_read_kv_entry():
    response = session.post(resolve_primary_gw_url("test/persist-orm/sequelize-write-kv-entry"),
                            json=entry)
    assert response.status_code == 200

    response = session.get(resolve_primary_gw_url("test/persist-orm/sequelize-read-kv-entry"),
