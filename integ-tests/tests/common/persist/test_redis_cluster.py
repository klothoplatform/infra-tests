...
def test_set_get_redis_cluster_entry():
    response = session.post(resolve_primary_gw_url("test/persist-redis/redis-cluster-set-entry"),
                            json=entry)
    assert response.status_code == 200

    response = session.get(resolve_primary_gw_url("test/persist-redis/redis-cluster-get-entry"),
