...
class AppBuilder:
    def __init__(self, directory: str, provider: str):
        self.app_name = f'{directory}-{shortuuid.ShortUUID().random(length=6)}'
        ...
        if directory.startswith("ts"):
            result: subprocess.CompletedProcess[bytes] = subprocess.run(["npm", "install", "--prefix", self.directory])
